...
fun Goals() {
    var goals by remember {
        mutableStateOf(
            listOf(
                Goal(1, "Take 10 push ups", 0.45f),
                Goal(2, "Workout 2 times", 0.75f),
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                OutlinedTextField(
                    ...
                    label = { Text(text = "Goal Title", color = DBrown) }
                )
            }
        },
        confirmButton = {
            TextButton(onClick = {
                ...
                Text("Save", color = DBrown)
            }
        },
        dismissButton = {
            TextButton(onClick = onDismiss) {
                Text("Cancel", color = DBrown)
            }
        }
    )
}
...
fun DeleteGoalDialog(
    ...
    AlertDialog(
        ...
        text = { Text(text = "Are you sure you want to delete \"${goal.target}\"?", color = DBrown) },
        containerColor = Swirl,
        confirmButton = {
            TextButton(onClick = onConfirm) {
                Text(text = "Delete", color = DBrown)
            }
        },
        dismissButton = {
            TextButton(onClick = onDismiss) {
                Text(text = "Cancel", color = DBrown)
            }
        }
    )
}
